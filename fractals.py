...
class Fractals:
	...
	def mandelbrot(self, minimun, maximun, width, height, maxIt):
		"""
	...
	def julia(self, c, exp, minimun, maximun, width, height, maxIt):
		"""
		...
		for w in range(height):
			row = []
			for h in range(width):
				row.append(self.in_julia(complex(cw,ch), c, exp, maxIt))
				cw += incReal
			imag.append(row)
			ch += incImag
			cw = minimun.real
